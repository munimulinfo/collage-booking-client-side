...
const IntroduceCollage = () => {
    ...
    return (
        <div className="grid lg:grid-cols-3 justify-center items-center gap-5 px-2 lg:px-16 py-20">
            {
                collageintro?.map(collage => <div key={collage?._id} className="card card-side bg-base-100 shadow-xl rounded-lg flex flex-col relative">
                    ...
                    <div className="card-actions absolute bottom-2 right-2">
                        <Link to={`/toydetails/${collage?._id}`}><button className="btn btn-sm btn-info">View Details</button></Link>
                    </div>
